...
def _rating_for_verdict(verdict: str) -> int | None:
    """Map the journal's verdict labels to a private 1–5 schema rating."""
    verdict_key = verdict.casefold()
    if "lost track" in verdict_key:
        return 5
    if "one more run" in verdict_key:
...
def review_page(
    ...
    date_iso = _value(review, "review_date_iso")
    ...
    image_alt = f"Cover art for {html.unescape(title)}"
    social_image_meta = (
    ...
    rating = _rating_for_verdict(raw_verdict)
    structured_data = {
        "@context": "https://schema.org",
        "@type": "Review",
        "name": html.unescape(title),
        "headline": f"{html.unescape(title)} review",
        ...
        "itemReviewed": {
            "@type": "VideoGame",
            "name": html.unescape(title),
            "gamePlatform": "Retroid Pocket 6",
            "operatingSystem": "Armada",
        },
    }
    if date_iso:
        structured_data["datePublished"] = date_iso
    ...
    if absolute_cover:
        structured_data["image"] = absolute_cover

    structured_json = json.dumps(structured_data, ensure_ascii=False).replace("</", "<\\/")
    article_date_meta = (
        f'<meta property="article:published_time" content="{html.escape(date_iso)}">'
        if date_iso
        else ""
    )
...
<head>
  ...
  {article_date_meta}
  <meta name="twitter:title" content="{title} review | {html.escape(SITE_NAME)}">
  <meta name="twitter:description" content="{description}">
  {social_image_meta}
  <link rel="canonical" href="{html.escape(canonical)}">
  <link rel="alternate" type="application/rss+xml" title="{html.escape(SITE_NAME)} RSS" href="../../feed.xml">
